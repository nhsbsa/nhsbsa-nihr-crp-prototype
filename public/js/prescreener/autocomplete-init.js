  ...
  function attach(inputSel, hiddenSel, src) {
    ...
    fetch(src).then(r => r.json()).then(list => {
      ...
      input.addEventListener('input', function () {
        ...
        box.innerHTML = current.map((item, i) => '<button type="button" data-i="' + i + '" class="nhsuk-button nhsuk-button--secondary" style="display:block;margin-bottom:6px;">' + item.name + '</button>').join('');
